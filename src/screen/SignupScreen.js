...
const SignupScreen = ({navigation}) =>{
    ...
    const renderButton = () => {
      ...
      const onSignUp = async () => {
        ...
        const signup = await firebase.auth().createUserWithEmailAndPassword(user.email, user.password).catch(function(e) {
            console.warn(e)
        })
        firebase.auth().signInWithEmailAndPassword(user.email, user.password);
        const update = firebase.auth().currentUser.updateProfile({
            displayName:name,
              photoURL:"https://github.com/TsiaYu1304/0721_wannashare/blob/master/src/img/IDphoto.png?raw=true"
          })
         const updatecoin = firebase.database().ref("Users").child(firebase.auth().currentUser.uid).set({
             sharecoin:0
         })
